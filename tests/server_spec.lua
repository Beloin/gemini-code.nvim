...
describe("geminicode.server.http - parser", function()
  ...
  it("builds a JSON response correctly", function()
    ...
    assert.is_truthy(resp:find("HTTP/1.1 200 OK"))
    assert.is_truthy(resp:find("Content-Type: application/json"))
    assert.is_truthy(resp:find('"ok"'))
  end)

  it("builds a 401 response", function()
    local resp = http.json_response(401, { error = "Unauthorized" })
    assert.is_truthy(resp:find("HTTP/1.1 401 Unauthorized"))
  end)
end)
